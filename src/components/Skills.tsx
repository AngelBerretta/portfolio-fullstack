...
const CATEGORY_META: Record<string, { path: string; accent: string; hex: string }> = {
  frontend: { path: '~/skills/frontend', accent: 'text-cyan-500', hex: '***REMOVED***22d3ee' },
  backend: { path: '~/skills/backend', accent: 'text-emerald-500', hex: '***REMOVED***4ade80' },
  tools: { path: '~/skills/tools', accent: 'text-violet-500', hex: '***REMOVED***c084fc' },
};
...
export default async function Skills() {
  ...
  const categories: TCategory[] = CATEGORY_ORDER
    ...
    .map((id) => ({
      ...
      hex: CATEGORY_META[id]?.hex ?? '***REMOVED***3b82f6',
