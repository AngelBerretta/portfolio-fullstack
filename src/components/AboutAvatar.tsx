    ...
    >
      <div className="relative">
        ...
        <div
          ...
          style={{ background: 'var(--bg-surface)', border: '1px solid rgba(59,130,246,0.4)' } as CSSProperties}
        >
          <span className="w-2.5 h-2.5 rounded-full bg-green-400 animate-pulse" />
          <span className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Open to work</span>
        </div>
      </div>

      {/* Trait pills — debajo del avatar, en fila */}
      <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 flex gap-2 flex-nowrap">
        {traits.map((t) => (
          <div
            key={t.text}
            className="flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs shadow-lg whitespace-nowrap"
            style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', color: 'var(--text-secondary)' } as CSSProperties}
          >
